        ...
        @forelse ($pengumuman as $pmn)
            <div class="col-md-6 col-lg-4 d-flex align-items-stretch">
                <div class="card shadow-sm w-100">
                    ...
                    <img src="{{ $pmn->foto ? asset('storage/' . $pmn->foto) : 'https://placehold.co/600x400/EEE/31343C?text=Pengumuman' }}" class="card-img-top" alt="{{ $pmn->judul }}">

                    <div class="card-body d-flex flex-column">
                        {{-- KATEGORI sebagai badge --}}
                        <div class="mb-2">
                            <span class="badge bg-primary">{{ $pmn->kategori }}</span>
                        </div>

                        {{-- JUDUL pengumuman --}}
                        <h5 class="card-title">{{ $pmn->judul }}</h5>

                        {{-- ISI pengumuman dibuat ringkas --}}
                        <p class="card-text flex-grow-1">
                            {{ Str::words($pmn->isi, 20, '...') }}
                        </p>
                    ...
                    <div class="card-footer text-muted">
                        <small>
                            Oleh: {{ $pmn->user->name ?? 'Admin' }} • {{ $pmn->created_at->translatedFormat('d F Y') }}
                        </small>
                    </div>
                </div>
            </div>
