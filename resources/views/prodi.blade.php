...
<body>
    ...
    <h2 class="section-title text-center mb-4">Daftar Program Studi dan Mata Kuliah</h2>

    <div class="accordion" id="prodiAccordion">
        @forelse ($prodi as $p)
            {{-- Setiap Program Studi menjadi satu item Accordion --}}
            <div class="accordion-item">
                <h2 class="accordion-header" id="heading-{{ $p->id }}">
                    <button class="accordion-button collapsed fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $p->id }}" aria-expanded="false" aria-controls="collapse-{{ $p->id }}">
                        {{ $p->nama_prodi }}
                    </button>
                </h2>
                <div id="collapse-{{ $p->id }}" class="accordion-collapse collapse" aria-labelledby="heading-{{ $p->id }}" data-bs-parent="#prodiAccordion">
                    <div class="accordion-body">
                        @php
                            // Kelompokkan mata kuliah berdasarkan semester
                            $matkulsPerSemester = $p->mataKuliahs->groupBy('semester')->sortKeys();
                        @endphp

                        @if($matkulsPerSemester->isEmpty())
                            <p class="text-muted">Belum ada data mata kuliah untuk program studi ini.</p>
                        @else
                            @foreach ($matkulsPerSemester as $semester => $matkuls)
                                <h5 class="mt-3 mb-2"><strong>Semester {{ $semester }}</strong></h5>
                                <ul class="list-group list-group-flush">
                                    @foreach ($matkuls as $matkul)
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            {{ $matkul->nama_matkul }}
                                            <span class="badge bg-primary rounded-pill">{{ $matkul->sks }} SKS</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @endforeach
        ...
        @empty
            {{-- Pesan ini ditampilkan jika variabel $prodi kosong --}}
            <div class="alert alert-info text-center" role="alert">
                Saat ini data program studi tidak tersedia.
            </div>
        @endforelse
    </div>
</div>
